...
//            ShowPanel();
        ...
        if (currentPosition == "center") {
            ...
            pos_width=pos.x();
            pos_height=pos.y()+2;
            if((pos_width+frmWidth) > myApp::DeskWidth)
                pos_width=((myApp::DeskWidth-frmWidth)/2);
            if(pos_height+frmHeight>myApp::DeskHeigth)
                pos_height=pos_height-frmHeight-nowWidget->height()-2;
            if(pos_height<0)pos_height=0;
//            QPoint pos = QPoint(pos_width, pos_height);
//            this->setGeometry(pos.x(), pos.y(), frmWidth, frmHeight);
